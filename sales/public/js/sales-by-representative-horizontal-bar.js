(function () {
    ...
    function createChart() {
        ...
        const chartId = window.salesByRepHorizontalBarChartId || 'salesByRepHorizontalBarChart';
        const canvas = document.getElementById(chartId);
        if (!canvas) return;

        const sourceData = window.salesByRepHorizontalBarData || null;
        const built = buildHorizontalBarData(sourceData);
        if (!built) {
            console.warn('Sales by sales representative horizontal bar chart data missing or malformed.');
            return;
        }

        const { labels, values } = maybeSortAscending(built.labels, built.values);

        const existing = window.salesByRepHorizontalBarChart;
        ...
        window.salesByRepHorizontalBarChart = new Chart(ctx, {
            type: 'bar',
            data: {
                labels,
                datasets: [
