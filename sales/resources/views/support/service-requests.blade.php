...
@section('content')
    ...
    <div class="card p-4 mt-4 sr-table-card"><div class="table-responsive"><table class="table sr-table align-middle mb-0"><thead><tr><th>Request</th><th>Ticket</th><th>Customer</th><th>Technician</th><th>Schedule</th><th>Priority</th><th>Status</th><th class="sr-action-header">Actions</th></tr></thead><tbody>
        @forelse($serviceRequests as $req)
            ...
            <tr id="service-request-row-{{ $req->request_id }}">
                ...
                <td class="sr-action-cell"><div class="sr-action-stack"><button type="button" class="btn btn-outline-primary sr-action-btn sr-view-btn js-service-request-view" data-request-id="{{ $req->request_id }}" data-bs-toggle="modal" data-bs-target="#serviceRequestViewModal"><i class="bi bi-eye" aria-hidden="true"></i>View</button>@if(in_array($requestStatus, ['pending', 'scheduled']))<button type="button" class="btn sr-action-btn sr-schedule-btn js-service-request-schedule" data-request-id="{{ $req->request_id }}" data-bs-toggle="modal" data-bs-target="#serviceRequestScheduleModal"><i class="bi bi-calendar3" aria-hidden="true"></i>{{ $requestStatus === 'scheduled' ? 'Reschedule' : 'Schedule' }}</button><button type="button" class="btn btn-outline-warning sr-action-btn sr-edit-btn" data-bs-toggle="modal" data-bs-target="#editRequest{{ $req->request_id }}"><i class="bi bi-pencil" aria-hidden="true"></i>Edit</button><form method="POST" action="{{ route('support.service-requests.cancel', $req) }}" onsubmit="return confirm('Cancel this service request?')">@csrf @method('PATCH')<input type="hidden" name="service_result" value="Cancelled by support staff"><button class="btn btn-outline-danger sr-action-btn sr-cancel-btn" type="submit"><i class="bi bi-x-circle" aria-hidden="true"></i>Cancel</button></form>@endif</div></td>
            </tr>
