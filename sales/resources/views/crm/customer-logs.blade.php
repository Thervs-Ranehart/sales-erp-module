...
$emailCount = $logsCollection
    ->where('communication_channel', 'Email')
    ->count();

$phoneCount = $logsCollection
    ->where('communication_channel', 'Phone')
    ->count();

$smsCount = $logsCollection
    ->where('communication_channel', 'SMS')
    ->count();

$emailPct = $totalForPage > 0
    ? round(($emailCount / $totalForPage) * 100)
    : 0;

$phonePct = $totalForPage > 0
    ? round(($phoneCount / $totalForPage) * 100)
    : 0;
                ...
                @php
                    $customerName = optional($log->customer)->display_name
                        ?? trim(
                            ...
                            . ' ' .
                            (optional($log->customer)->last_name ?? '')
                        );
            ...
            @forelse($logs as $log)
                ...
                <tr>
                    ...
                    <td class="text-center">
                        ...
                        <form
                            method="POST"
                            action="{{ route('crm.logs.status.update', $log) }}"
                            class="d-inline">
...
<div
    class="modal fade"
    id="viewLogModal{{ $log->communication_id }}"
    tabindex="-1">
