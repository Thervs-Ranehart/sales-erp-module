...
@section('content')
    ...
    <main class="w-full flex items-center" style="margin:-1rem -1rem 0;width:calc(100% + 2rem);padding:32px 48px;height:258px;max-height:258px;background:linear-gradient(90deg,#128B99 0%,#1CE5BD 100%);border-radius:0;">
        <div class="d-flex flex-column h-100 w-100"><h1 class="text-white font-semibold text-[45px] leading-tight">Sales Recommendations</h1><div class="flex-grow-1 d-flex align-items-center"><p class="text-white mb-0" style="font-family:'Poppins',sans-serif;font-size:18px;font-weight:600;line-height:1.6;opacity:.95;">Turn sales reports, target performance, and forecast trends into actionable recommendations for sales, marketing, and inventory planning.</p></div><div><a href="{{ route('forecasting.export', array_merge(['type'=>'recommendations','format'=>'csv'], request()->query())) }}" class="hero-action-btn" style="padding:.75rem 1.6rem;"><i class="bi bi-download me-2"></i>Export Recommendations</a><a href="{{ route('forecasting.export', array_merge(['type'=>'recommendations','format'=>'print'], request()->query())) }}" target="_blank" class="hero-secondary-btn ms-2"><i class="bi bi-printer me-2"></i>Print / PDF</a></div></div>
    </main>

    <section class="mt-4" aria-label="Recommendation KPI summary"><div class="row g-4">@foreach($kpis as $kpi)<div class="col-12 col-md-6 col-lg-4"><article class="card border-0 shadow-sm h-100 px-3 py-2" style="background:linear-gradient(145deg,#fff,#f8fafc);min-height:92px;"><div class="d-flex justify-content-between align-items-center mb-1"><span class="small fw-semibold text-muted">{{ $kpi['label'] }}</span><span class="rounded-circle d-flex align-items-center justify-content-center" style="width:32px;height:32px;background:{{ $kpi['accent'] }}"><i class="bi bi-{{ $kpi['icon'] }} {{ $kpi['tone'] }}"></i></span></div><div class="fs-4 fw-bold {{ $kpi['tone'] }}">{{ $kpi['value'] }}</div><p class="small text-muted mb-0 mt-1">Current review queue</p></article></div>@endforeach</div></section>
    ...
    <div class="col-md-6"><label class="form-label">Workflow Status</label><select class="form-select" name="implementation_status">@foreach(['New','Under Review','Approved','Rejected','In Progress','Completed'] as $option)<option @selected($row['status']===$option)>{{ $option }}</option>@endforeach</select></div>
    <div class="col-md-6"><label class="form-label">Assigned Employee</label><select class="form-select" name="assigned_to"><option value="">Department assignment</option>@foreach($employees as $employee)<option value="{{ $employee->employee_id }}" @selected($row['assigned_to']===$employee->employee_id)>{{ $employee->full_name }} — {{ $employee->department }}</option>@endforeach</select></div>
    <div class="col-md-6"><label class="form-label">Assigned Department</label><input class="form-control" name="assigned_department" value="{{ $row['responsible_team'] }}"></div><div class="col-md-6"><label class="form-label">Due Date</label><input class="form-control" type="date" name="due_date" value="{{ $row['due_date'] }}"></div>
    <div class="col-12"><label class="form-label">Decision / Rejection Notes</label><textarea class="form-control" name="decision_notes">{{ $row['decision_notes'] }}</textarea></div><div class="col-12"><label class="form-label">Implementation Outcome</label><textarea class="form-control" name="outcome">{{ $row['outcome'] }}</textarea><div class="form-text">Required when marking an action completed.</div></div>
    </div></div><div class="modal-footer"><button class="btn btn-outline-secondary" type="button" data-bs-dismiss="modal">Cancel</button><button class="btn btn-primary">Update Workflow</button></div></form></div></div>@endif @endforeach
